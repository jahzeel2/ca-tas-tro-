...
$(document).ready(function () {
    ...
    $("#form-filtros").ajaxForm({
        ...
        error: function (err) {
            //500 no tiene datos
            //417 Error de consulta u otro error
            if (err.status === 500) {
                alerta('Mapa Temático - Sin datos', 'No se encontraron datos', 3);
            } else if (err.status === 417) {
                alerta('Mapa Temático - Error', 'Error en la consulta.', 3);
            }
            hideLoading();
        }
    });
    ...
    $("#geografico").click(function () {
        ...
        new Promise(function (resolve) {
            if (!mapa) {
                mapa = new MapaController(4, "mapaMT", true, false, true, false, true, true, false, false, false, false, true);
    ...
    $("#cerrar").click(function () {
        ...
        alerta('Mapas Temáticos: Filtros', msj, 2, function () {
            $('.modal-backdrop').remove();
            $("#myModalMT").modal('hide');
        });
        return false;
    ...
    $("#ComponenteULAlfa").on('click', 'li', function () {
        ...
        $.ajax({
            type: 'POST',
            url: BASE_URL + 'MapasTematicos/GetAtributosByComp',
            dataType: 'json',
    ...
    $("#buscar").keyup(function () {
        ...
        if (texto) {
            ...
            var spans = $("#ComponenteULAlfa").find("li").find("label");
    ...
    $("#aceptarGeografico").click(function (evt) {
        if (!mapa.obtenerDibujos().length && !mapa.obtenerSeleccion().seleccion.length && !$("#OperacionesULGeografico li.seleccionado").length) {
            alerta('Mapas Temáticos: Filtro Geográfico', ['Debe seleccionar un filtro para continuar.'], 2);
            return;
        }
    ...
    $("#btnBorrar").on('click', function () {
        $("#filtros .seleccionado").each(function (inx, val) {
            ...
            $.ajax({
                "url": "MapasTematicos/RemoveFilter?idFilter=" + idFiltro,
                "async": false,
...
function alerta(titulo, mensaje, tipo, fn) {
    var cls = "";
    if (fn) {
        $("#btnAceptarInfoMTFiltros").one('click', fn);
        $("#ModalInfoMTFiltros .modal-footer").show();
    } else {
        $("#ModalInfoMTFiltros .modal-footer").hide();
    }
    switch (tipo) {
        case 1:
            cls = "alert-success";
    ...
    $("#MensajeInfoMTFiltros").removeClass("alert-info alert-warning alert-danger alert-success").addClass(cls);
    $("#TituloInfoMTFiltros").html(titulo);
    $("#DescripcionInfoMTFiltros").html(mensaje);
    $("#ModalInfoMTFiltros").modal('show');
    return false;
}
...
function generarFiltroColeccion(divTexto) {
    divTexto.appendChild(crearElementoHtml('input', { type: 'hidden', name: 'Filtros[' + liEditado + '].FiltroColeccion', value: $('.ColeccionId:checked').val(), class: 'fcoleccion' }));
